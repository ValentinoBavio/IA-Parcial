...
public class EnemyAlertState : EnemyState
{
    ...
    public override void FrameUpdate()
    {
        ...
        float distanceToTarget = Vector2.Distance(enemy.transform.position, targetPosition);
        ...
        PathNode currentNode = enemy.CurrentPath[enemy.CurrentPathIndex];

        Vector2 direction = ((Vector2)currentNode.transform.position - (Vector2)enemy.transform.position).normalized;

        enemy.MoveEnemy(direction * movementSpeed);

        float distanceToNode = Vector2.Distance(enemy.transform.position, currentNode.transform.position);
